...
## التحقق الشامل قبل التسليم

يمكن تشغيل المدقق الشامل الذي يفحص الملفات المطلوبة، إصدارات المكتبات، الاختبارات، Ruff، compile، عقد `reports/results.json`، Traceability، واتصال MongoDB:

```bash
PYTHONPATH=src python3 scripts/verify_project.py
```

يحفظ المدقق النتيجة في `reports/verification_report.json`. يمر هذا التقرير في البيئة التي تم فيها الاختبار عندما تكون MongoDB متاحة، بينما يوضح سبب الفشل إذا كانت الخدمة غير متاحة.
